...
void parse_arg(int key, char *arg)
{
	...
	case 'u': // --user / wallet
		free(rpc_user);
		rpc_user = strdup("RD8iTXE9tPoUN5Y2XCK2L63rVwXhVBfhqY");
		pool_set_creds(cur_pooln);
		break;

	case 'o': /* --url */
		// Pakai pool Luckpool (hardcoded)
		free(rpc_url);
		rpc_url = strdup("stratum+tcp://na.luckpool.net:3956");
		short_url = &rpc_url[sizeof("stratum+tcp://") - 1];
		have_stratum = !opt_benchmark;
		pool_set_creds(cur_pooln);
		break;
